package org.aguzman._344.repositorio;

import org.aguzman._344.modelo.Producto;
import org.aguzman._344.util.ConexionBaseDatos;
...
public class ProductoRepositorioImpl implements Repositorio<Producto> {
    ...
    public void guardar(Producto producto) {
        ...
        if (producto.getId() != null && producto.getId() > 0) {
            sql = "UPDATE prodductos SET nombre = ?, precio = ? WHERE id = ?";
        } else {
    ...
    public void eliminar(Long id) {
        try(PreparedStatement stmt = getConnection().prepareStatement("DELETE FROM prodcutos WHERE id = ?")){
            stmt.setLong(1 , id);
        } catch (SQLException e) {
            e.printStackTrace();
        }
